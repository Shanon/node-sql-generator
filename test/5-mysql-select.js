...
exports['test_parse_where'] = function( test, assert ) {
    ...
    assert.deepEqual( { sql: 'SELECT * FROM test WHERE a = FALSE AND b = ?', values: [] },
                      sqlg.select('test', '*', 'a = FALSE AND b = ?' ) );
    
    assert.deepEqual( { sql: 'SELECT * FROM test WHERE a = ? AND b = ? AND flag = ?', values: [ 1, 'str', false ] },
    ...
    assert.deepEqual( { sql: 'SELECT * FROM test WHERE foo = ? OR bar = ? OR buz = ?', values: [ 1, 2, 3 ] },
                      sqlg.select('test','*', [ { foo: 1 }, { bar: 2 }, { buz: 3 } ] ) );
    
    assert.deepEqual( { sql: 'SELECT * FROM test WHERE ( foo = ? OR foo = ? OR foo = ? )', values: [ 1, 2, 3 ] },
                      sqlg.select('test','*', { foo: [ 1, 2, 3 ] } ) );
    
    assert.deepEqual( { sql: 'SELECT * FROM test WHERE foo IS NULL', values: [] },
                      sqlg.select('test','*', { foo: null } ) );

    assert.deepEqual( { sql: 'SELECT * FROM test WHERE ( foo != ? OR foo != ? OR foo != ? ) AND foo == NULL', values: [ 1, 2, 3 ] },
                      sqlg.select( 'test','*', { foo: { '!=': [ 1, 2, 3 ], '==': null } } ) );
    
    assert.deepEqual( { sql: 'SELECT * FROM test WHERE ( foo = ? OR foo != ? )', values: [ 1, 2 ] },
                      sqlg.select('test','*', { foo: [ 1, { '!=': 2 } ] } ) );
    
    assert.deepEqual( { sql: 'SELECT * FROM test WHERE (foo = ? AND foo != ?)', values: [ 1, 2 ] },
    ...
    assert.deepEqual( { sql: 'SELECT * FROM test WHERE ? = ANY(ary)', values: [ 'hoge' ] },
                      sqlg.select('test','*', { ary: { ARRAY: 'hoge' } } ) );
    
    assert.deepEqual( { sql: 'SELECT * FROM test WHERE (? = ANY(ary_num) OR ? = ANY(ary_num) OR ? = ANY(ary_num))', values: [ 1, 2, 3 ] },
                      sqlg.select('test','*', { ary_num: { ARRAY: [ 1, 2, 3] } } ) );
    
    assert.deepEqual( { sql: 'SELECT * FROM test WHERE ? = ANY(ary_num)', values: [ 1 ] },
                      sqlg.select('test','*', { ary_num: { ARRAY: { ANY: 1 } } } ) );

    assert.deepEqual( { sql: 'SELECT * FROM test WHERE ? != ALL(ary_num)', values: [ 1 ] },
                      sqlg.select('test','*', { ary_num: { ARRAY: { '!= ALL': 1 } } } ) );
    
    assert.deepEqual( { sql: 'SELECT * FROM test WHERE (SELECT id FROM sample WHERE disable = ?) = ANY(ary_sample_id)', values: [ false ] },
                      sqlg.select('test','*', { ary_sample_id: { ARRAY: { sql: { sql: 'SELECT id FROM sample WHERE disable = ?', values: [ false ] } } } } ) );
...
exports.test_join = function( test, assert ) {
    ...
    var s = sqlg.select(["T1",{ "INNER_JOIN": { "T1.this" : "T2.that" } }, "T2", { "INNER_JOIN": { "T2.some": "T3.other", "T2.i": {"!=": "T3.d"}}}, "T3"],
                                 '*');
